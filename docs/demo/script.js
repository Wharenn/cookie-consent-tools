...
var cct = window.cookieConsentTools.default;
...
registerClickHandlerIfExists('show-box', function (e) {
  e.preventDefault();
  writeSeparator();
  write('User Action: Showing Box');
  cct.consentBox.show();
});
registerClickHandlerIfExists('hide-box', function (e) {
  e.preventDefault();
  writeSeparator();
  write('User Action: Hiding Box');
  cct.consentBox.hide();
});
registerClickHandlerIfExists('reset-button', function (e) {
  e.preventDefault();
  writeSeparator();
  write('User Action: Reseting consent');
  cct.cookieManager.resetConsent();
});
registerClickHandlerIfExists('approve-button', function (e) {
  e.preventDefault();
  writeSeparator();
  write('User Action: Giving consent');
  cct.cookieManager.approve();
});
registerClickHandlerIfExists('decline-button', function (e) {
  e.preventDefault();
  writeSeparator();
  write('User Action: Revoking consent');
  cct.cookieManager.decline();
});
registerClickHandlerIfExists('has-consent-button', function (e) {
  e.preventDefault();
  writeSeparator();
  write('Has consent? ' + (cct.cookieManager.hasConsent() ? 'Yes' : 'Nope'));
});
registerClickHandlerIfExists('has-choice-button', function (e) {
  e.preventDefault();
  writeSeparator();
